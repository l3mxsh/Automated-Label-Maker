...
// Sticker pages: dedicated pages after label pages, tiled with no gap, within margin
$stickerCols = max(1, (int)floor((210 - 2*$margin) / STK_W));
$stickerRows = max(1, (int)floor((297 - 2*$margin) / STK_H));
$perStickerPage = $stickerCols * $stickerRows;

// Center the sticker grid on the page
$stkGridW = $stickerCols * STK_W;
$stkGridH = $stickerRows * STK_H;
$stkStartX = $margin + ((210 - 2*$margin) - $stkGridW) / 2;
$stkStartY = $margin + ((297 - 2*$margin) - $stkGridH) / 2;

$stickerPages  = $totalStickers > 0 ? (int)ceil($totalStickers / $perStickerPage) : 0;
$totalPages    = $labelPages + $stickerPages;
if ($totalPages === 0) $totalPages = 1;

$stickerLayout = [];
$stickerIdx    = 0;
for ($p = 0; $p < $stickerPages; $p++) {
    $pageNum = $labelPages + $p + 1;
    for ($row = 0; $row < $stickerRows; $row++) {
        for ($col = 0; $col < $stickerCols; $col++) {
            $tx = $stkStartX + $col * STK_W;
            $ty = $stkStartY + $row * STK_H;
            $entry = ['page'=>$pageNum,'cell'=>$stickerIdx,'x_mm'=>round($tx,4),'y_mm'=>round($ty,4),'w_mm'=>STK_W,'h_mm'=>STK_H,'sticker_name'=>null,'img_path'=>null];
...
echo json_encode([
    ...
    'sticker_pages'    => $stickerPages,
    'slots_per_page'   => $slotsPerPage,
    'per_sticker_page' => $perStickerPage,
    'label_w_mm'       => $lw,
    'label_h_mm'       => $lh,
